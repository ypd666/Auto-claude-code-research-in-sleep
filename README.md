...
- **2026-03-23** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔧 **3 skills integrated into core workflows**: `/training-check` auto-runs after experiments (W1.5 + W2), `/result-to-claim` bridges W2→W3 with structured claims, `/ablation-planner` auto-designs ablation studies after main experiments. All skip silently if unavailable. Community contributions by [@JingxuanKang](https://github.com/JingxuanKang). Plus: **watchdog.py** (server-side monitoring), **3 new templates** (idea candidates, findings, experiment log), **session recovery guide**
