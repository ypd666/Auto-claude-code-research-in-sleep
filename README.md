...
- **2026-03-18** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔗 **[Workflow 1.5](skills/experiment-bridge/SKILL.md)** — new `/experiment-bridge`: plan → Claude writes code → **GPT-5.4 reviews code** → sanity check → deploy → collect. Cross-model code review before GPU deployment (`code review: true` by default). 📊 **W&B integration** — auto `wandb.log()` when `wandb: true`
